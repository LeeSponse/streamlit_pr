...
def main() :
    ...
    status = st.radio('정렬방법 선택하기', my_order)
    ...
    #체크하면, 헤드 5개 보여주고, 해제하면 안보여주게.
    if st.checkbox('헤드 5개 보기') : # ,True로 쓰면 자동으로 체크되어있다. 기본은 False 로 되어있음
    ...
    my_choice = st.selectbox('좋아하는 언어 선택하기.', Language)

    if my_choice == Language[0] or my_choice == Language[2]:
        st.text('정말 재미있는 언입니다.')
    elif my_choice == Language[3] or my_choice == Language[5]:
        st.text('배우고 싶습니다.')
    else:
        st.text('오래된 언어입니다.')

    #멀티셀렉트 : 여러개중에, 여러개를 선택하게 할때.

    # 유저가 선택한 컬럼을, 데이터프레임으로 보여주되,
    # 아무것도 선택안하면 ,아무것도 나오지 않게 하기.
    choice_list = st.multiselect('원하는 컬럼을 선택하기.', df.columns)
    
    # len() == 0 , ==[], list(), !=0(비어있지 않은 0이 아닌)
    ...
    # 선택한 나이가 웹 화면에 출력되도록 하기. '선택한 나이는 {33}세 입니다.'
